...
func (s *serviceManager) GetStatus() *Status {
	...
	return &Status{Field: map[string]string{
		...
		"Tx.PageCount":     strconv.FormatInt(st.TxStats.PageCount, 10),
		"Tx.PageAlloc":     strconv.FormatInt(st.TxStats.PageAlloc, 10),
		"Tx.CursorCount":   strconv.FormatInt(st.TxStats.CursorCount, 10),
		"Tx.NodeCount":     strconv.FormatInt(st.TxStats.NodeCount, 10),
		"Tx.NodeDeref":     strconv.FormatInt(st.TxStats.NodeDeref, 10),
		"Tx.Rebalance":     strconv.FormatInt(st.TxStats.Rebalance, 10),
		"Tx.RebalanceTime": st.TxStats.RebalanceTime.String(),
		"Tx.Split":         strconv.FormatInt(st.TxStats.Split, 10),
		"Tx.Spill":         strconv.FormatInt(st.TxStats.Spill, 10),
		"Tx.SpillTime":     st.TxStats.SpillTime.String(),
		"Tx.Write":         strconv.FormatInt(st.TxStats.Write, 10),
